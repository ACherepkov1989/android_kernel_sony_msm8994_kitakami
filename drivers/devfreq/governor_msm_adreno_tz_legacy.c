				...
				u32 *flag)
{
	...
	if (priv == NULL) {
		pr_err("%s: ERROR: No adreno tz data!!!!\n", __func__);
		return 0;
	}

	if (devfreq == NULL) {
		pr_err("%s: ERROR: No adreno tz devfreq!!!!\n", __func__);
		return 0;
	}

	if (devfreq->profile == NULL) {
		pr_err("%s: ERROR: No adreno tz profile!!!!\n", __func__);
		return 0;
	}

	if (devfreq->profile->get_dev_status == NULL) {
		pr_err("%s: ERROR: No adreno tz get_dev_status!!!!\n", __func__);
		return 0;
	}
/*
	if (devfreq->dev == NULL) {
		pr_err("%s: ERROR: No adreno tz devfreq dev!!!!\n", __func__);
		return 0;
	}
*/
	if (devfreq->dev.parent == NULL) {
		pr_err("%s: ERROR: No adreno tz devfreq dev parent!!!!\n", __func__);
		return 0;
	}
